...
class GenerateGraph(nn.Module):
    def __init__(self, fps_points, neighbors):
        super().__init__()
        self.fps_points = fps_points
        self.neighbors = neighbors

    def forward(self, xyz, x):
        fps_idx = farthest_point_sample(xyz.contiguous(), self.fps_points)
...
class SimMLP(nn.Module):
    def __init__(self, in_channels, out_channels, fps_points, kneighbor=20,
                 la_num=1, ca_num=1, res_expansion=1., bias=False, act="relu"):
        super().__init__()
        self.gg = GenerateGraph(fps_points, kneighbor)
        self.la = Local(
...
class BaseBlock(nn.Module):
    def __init__(self, in_channels, out_channels, fps_points, k_neighbors,
...
class Moduletemp(nn.Module):
    def __init__(self, cfg=None, token=6, t_channels=256, embed_dim=64, fps_points=1024,
                 ...
                 activation="gelu", heads=2, num_tf=2, res_expansion=1.0
                 ):
        super().__init__()
